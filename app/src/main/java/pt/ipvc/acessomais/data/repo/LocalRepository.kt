...
class LocalRepository(private val localDao: LocalDao) {
    ...
    fun getLocais(email: String): Flow<List<Local>> =
        localDao.getLocaisByUser(email).map { list ->
            list.map { it.toDomain() }
        }

    fun getAllLocais(): Flow<List<Local>> =
        localDao.getAllLocais().map { list ->
            list.map { it.toDomain() }
        }

    suspend fun saveLocal(local: Local) {
        // Corrigido para insertLocal para bater com o LocalDao
        localDao.insertLocal(local.toEntity())
    }

    suspend fun deleteLocal(local: Local) {
        // Corrigido para deleteLocal para bater com o LocalDao
